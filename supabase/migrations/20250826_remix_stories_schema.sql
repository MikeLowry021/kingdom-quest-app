...
-- Insert some sample original quests for testing
INSERT INTO original_quests (title, description, scripture_reference, content, age_tiers, duration_minutes, category, tags, difficulty_level, is_official) VALUES
(
  'Noah''s Ark: Faith in the Storm',
  'Experience Noah''s obedience to God and His faithfulness through the great flood.',
  'Genesis 6-9',
  '{
    "scenes": [
      {
        "id": "scene1",
        "title": "God''s Warning",
        "description": "God speaks to Noah about the coming flood",
        "content": "God saw the wickedness in the world and chose Noah, a righteous man, to build an ark.",
        "questions": [
          {
            "question": "Why did God choose Noah?",
            "options": ["He was wealthy", "He was righteous", "He was strong", "He was young"],
            "correct": 1
          }
        ]
      },
      {
        ...
        "description": "Noah obeys God and begins construction",
        "content": "Despite ridicule from others, Noah faithfully built the ark according to God''s instructions.",
        "questions": [
          {
            "question": "How did Noah respond to God''s command?",
            "options": ["He questioned it", "He obeyed immediately", "He asked for proof", "He delayed"],
            "correct": 1
          }
        ]
      }
    ],
    "lesson": "Obedience to God requires faith, even when we don''t understand His plans.",
    "memory_verse": "By faith Noah, when warned about things not yet seen, in holy fear built an ark to save his family. - Hebrews 11:7"
  }',
  ARRAY['early_childhood', 'elementary', 'middle_school'],
  20,
  'old_testament',
  ARRAY['faith', 'obedience', 'flood', 'animals'],
  'medium',
  true
),
(
  'David and Goliath: Courage Through Faith',
  'Discover how young David defeated the giant Goliath through trust in God.',
  '1 Samuel 17',
  '{
    "scenes": [
      {
        ...
        "description": "Goliath taunts the army of Israel",
        "content": "For forty days, the giant Goliath challenged Israel''s army, but no one dared to fight him.",
        "questions": [
          {
            "question": "How long did Goliath challenge Israel?",
            "options": ["Seven days", "Twenty days", "Forty days", "One year"],
            "correct": 2
          }
        ]
      },
      {
        "id": "scene2",
        "title": "David''s Faith",
        "description": "Young David volunteers to fight",
        "content": "David said, ''The Lord who rescued me from the paw of the lion and the bear will rescue me from this Philistine.''",
        "questions": [
          {
            "question": "What gave David courage to fight Goliath?",
            "options": ["His strength", "His weapons", "His faith in God", "His anger"],
            "correct": 2
          }
        ]
      }
    ],
    "lesson": "With God, we can overcome any challenge, no matter how impossible it seems.",
    "memory_verse": "The Lord does not save with sword and spear; for the battle is the Lord''s. - 1 Samuel 17:47"
  }',
  ARRAY['elementary', 'middle_school', 'high_school'],
  18,
  'old_testament',
  ARRAY['courage', 'faith', 'giant', 'victory'],
  'medium',
  true
),
(
  'The Good Samaritan: Love Your Neighbor',
  'Learn about compassion and loving others through Jesus'' parable.',
  'Luke 10:25-37',
  '{
    "scenes": [
      {
        "id": "scene1",
        "title": "The Question",
        "description": "A lawyer asks Jesus about eternal life",
        "content": "A lawyer asked Jesus, ''Teacher, what must I do to inherit eternal life?''",
        "questions": [
          {
            "question": "Who asked Jesus the question?",
            "options": ["A doctor", "A lawyer", "A priest", "A farmer"],
            "correct": 1
          }
        ]
      },
      {
        "id": "scene2",
        "title": "The Parable",
        "description": "Jesus tells the story of the Good Samaritan",
        "content": "Jesus told of a man beaten by robbers, ignored by religious leaders, but helped by a Samaritan.",
        "questions": [
          {
            "question": "Who helped the injured man?",
            "options": ["A priest", "A Levite", "A Samaritan", "A soldier"],
            "correct": 2
          }
        ]
      }
    ],
    "lesson": "We should show love and compassion to everyone, regardless of who they are.",
    "memory_verse": "Love your neighbor as yourself. - Luke 10:27"
  }',
  ARRAY['early_childhood', 'elementary', 'middle_school'],
  15,
  'parables',
  ARRAY['love', 'compassion', 'neighbor', 'kindness'],
  'easy',
  true
);
...
CREATE TRIGGER update_original_quests_updated_at BEFORE UPDATE ON original_quests
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_remix_quests_updated_at BEFORE UPDATE ON remix_quests
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Add trigger to automatically create attribution records
CREATE OR REPLACE FUNCTION create_remix_attribution()
RETURNS TRIGGER AS $$
BEGIN
    -- Create attribution record for the remix
    INSERT INTO remix_attributions (
        remix_quest_id,
        original_quest_id,
        parent_remix_id,
        attribution_level,
    ...
    SELECT 
        NEW.id,
        NEW.original_quest_id,
        NULL, -- This is a direct remix, not remix-of-remix
        1,
        COALESCE(u.raw_user_meta_data->>'full_name', u.email, 'Anonymous Creator'),
        NEW.creator_id,
        oq.scripture_reference
    FROM auth.users u, original_quests oq
    WHERE u.id = NEW.creator_id 
    AND oq.id = NEW.original_quest_id;
    
    RETURN NEW;
END;
$$ language 'plpgsql';

CREATE TRIGGER create_remix_attribution_trigger 
    AFTER INSERT ON remix_quests
    FOR EACH ROW EXECUTE FUNCTION create_remix_attribution();
