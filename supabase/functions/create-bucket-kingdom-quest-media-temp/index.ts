...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'kingdom-quest-media',
        name: 'kingdom-quest-media',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["image/*", "audio/*", "video/*", "application/pdf"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for kingdom-quest-media" ON storage.objects FOR SELECT USING (bucket_id = 'kingdom-quest-media');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for kingdom-quest-media" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'kingdom-quest-media');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for kingdom-quest-media" ON storage.objects FOR UPDATE USING (bucket_id = 'kingdom-quest-media');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for kingdom-quest-media" ON storage.objects FOR DELETE USING (bucket_id = 'kingdom-quest-media');`
    ];
